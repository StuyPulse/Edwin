    ...
    public EightBallTwoRdvsThreeTrenchAutonCommand(Intake intake, Shooter shooter, Funnel funnel, Chimney chimney, Drivetrain drivetrain, LEDController  controller) {
        ...
        addCommands(
            ...
            new ParallelCommandGroup(
                new TimeoutCommand(new FeedBallsCommand(funnel, chimney), 2.0),
                new SequentialCommandGroup(
                    new WaitCommand(0.5),
                    new LEDSetCommand(Color.ORANGE_SOLID, controller),
                    new DrivetrainMovementCommand(drivetrain, 0, DISTANCE_TO_RDVS_IN_FEET).withTimeout(3.0)
                )
            ),

            // new LEDSetCommand(Color.ORANGE_SOLID, controller),
            // new DrivetrainMovementCommand(drivetrain, 0, DISTANCE_TO_RDVS_IN_FEET).withTimeout(15.0),
